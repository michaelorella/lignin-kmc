...
options shown below, or by using a configuration file. These options can be viewed by entering the help command:
 ...
 `> create_lignin -h`
    ...
    usage: _jb_unittest_runner.py [-h] [-a ADD_RATES] [-b] [-c CONFIG]
                                  [-d OUT_DIR] [-dy] [-e] [-f OUTPUT_FORMAT_LIST]
    ...
    optional arguments:
      ...
      -e, --bypass_b1_error
                            Currently, there is a problem in the function for generating an rdKit molecule (which is required to produce SMILES strings and for JSON, PNG, and SVG output), which by default causes the program to exit with a warning message. This flag instead has the program try again instead of exit, as this type of bond only occasionally occurs. It will only retry once, to avoid an infinite loop.
